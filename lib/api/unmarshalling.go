...
func Unmarshalling(router *jwt_http_router.Router, conf config.Config, marshaller *marshaller.Marshaller, configurableService *configurables.ConfigurableService, deviceRepo DeviceRepository) {
	resource := "/unmarshalling"

	normalizeRequest := func(request *UnmarshallingRequest, jwt jwt_http_router.Jwt) error {
	...
	unmarshal := func(request UnmarshallingRequest) (interface{}, error) {
		return marshaller.UnmarshalOutputs(*request.Protocol, request.Service, request.Message, request.CharacteristicId)
	}
